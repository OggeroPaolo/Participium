...
//Add update report category
router.patch("/reports/:reportId/category/:caterogyId", [
  param("reportId").isInt().withMessage("Report ID must be a valid integer"),
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const reportId = Number(req.params.reportId);
      const caterogyId = Number(req.params.categoryId);

      await reportDAO.updateReportCategory(reportId, caterogyId);

      return res.status(200).json({
        message: "Report category updated succesfully"
      });
    } catch (error: any) {
